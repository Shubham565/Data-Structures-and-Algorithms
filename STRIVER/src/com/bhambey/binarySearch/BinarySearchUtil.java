...
public class BinarySearchUtil {
	...
	private static int getGasStationCount(int[] arr, double dist) {

		int n = arr.length;
		int count = 0;

		for (int i = 1; i < n - 1; i++) {
			int numberOfStationsInBetween = (int) ((arr[i] - arr[i - 1]) / dist);
			if ((arr[i] - arr[i - 1]) == (dist * numberOfStationsInBetween)) {
				numberOfStationsInBetween--;
			}
			count += numberOfStationsInBetween;
		}
		return count;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////
		...
		while (high - low > maxDiff) {
			...
			int count = getGasStationCount(arr, mid);
			if (count > k) {
				low = mid;
			} else {
				high = mid;
	...
	public static int kthElementTwoSortedArrays(ArrayList<Integer> a, ArrayList<Integer> b, int m, int n,
			int k) {
		 ...
		 * arrays.
		 * 
		 * Reason: We are applying binary search on the range [max(0, k-n2), min(k,
		 * n1)]. The range length <= min(m, n).
		 * 
		 * Space Complexity: O(1), as we are not using any extra space to solve this
		 * problem.
		 */
		
		if(m > n) return kthElementTwoSortedArrays(b, a, n, m, k);
		...
		while(low <= high)
		{
			...
			int l2 = (mid1 - 1 >= 0) ? b.get(mid2 - 1) : Integer.MIN_VALUE;
			int r1 = (mid1 <= m) ? a.get(mid1) : Integer.MAX_VALUE;
			int r2 = (mid2 <= n) ? a.get(mid2) : Integer.MAX_VALUE;
